...
{
    public class CpDapperRepository : DapperRepositoryBase<Cp>, ICpRepository
    {
        #region Constructor

        public CpDapperRepository(IConfiguration configuration) : base(configuration)
        {
        }
        ...
        public int AddChip(string userId, int addedChips, ChipProcessNames chipProcessName)
        {
            ...
            Cp Cp = Find(p => p.UserId == userId).FirstOrDefault();
            if (Cp == null)
            {
        ...
        {
            return Connection.Query<Duel>("select * from [Duels] as [d] where [d].[DuelId]=@DuelId", new { DuelId = duelId }).FirstOrDefault();
        }

        private List<DuelInfo> GetDuelInfo(int duelId)
        {
            string sql = @"select [di].[TrueAnswer], [di].[FounderUserAnswer] as [FounderTrueAnswerCount], [di].[CompetitorUserAnswer] as [CompetitorUserAnswer]
                           from [DuelInfoes] as [di] where [di].[DuelId]=@DuelId";
            return Connection.Query<DuelInfo>(sql, new { DuelId = duelId }).ToList();
        }

        private void UpdateDuelCp(Duel duel)
        {
            Connection.Execute("update Duels d set d.Cp=@Cp where d.DuelId=@DuelId", new { Cp = -duel.Cp, DuelId = duel });// TODO: Bu işlem duel repository de yapılmalı
        }
        ...
        public void ChipDistribution(int duelId)
        {
            ...
            {
                UpdateDuelCp(duel);
                var duelUserInfo = Connection
                                .Query<dynamic>("Select [di].[TrueAnswer], [di].[FounderUserAnswer], [di].[CompetitorUserAnswer] from [DuelInfoes] as [di] where [di].[DuelId]=@DuelId", new
                                {
                                    DuelId = duelId
                                }).Select(di => new
                                {
                ...
                if (founderTrueAnswerCount > competitorUserAnswerCount) this.AddChip(duel.FounderUserId, duel.Cp, ChipProcessNames.Win);//Kurucu kazanırsa
                else if (competitorUserAnswerCount > founderTrueAnswerCount) this.AddChip(duel.CompetitorUserId, duel.Cp, ChipProcessNames.Win);//Rakib kazanırsa
                else if (competitorUserAnswerCount == founderTrueAnswerCount)//Berabera biterse chipler geri iade ediliyor
                {
                    int chips = duel.Cp / 2;//düellodaki chipleri ikiye bölüp başlaştırdık
        ...
        public void ChipDistribution(int duelId, byte FounderScore, byte CompetitorScore)
        {
            ...
            if (duel.Cp > 0)
            {
                ...
                var duelUserInfo = Connection
                                ...
                                .ToList();
                if (FounderScore > CompetitorScore) this.AddChip(duel.FounderUserId, duel.Cp, ChipProcessNames.Win);//Kurucu kazanırsa
        ...
        {
            return Find(p => p.UserId == userId).Select(p => p.CpAmount).FirstOrDefault();
        }
        ...
        public int RemoveChip(string userId, int diminishingChips, ChipProcessNames chipProcessName)
        {
            ...
            if (Cp.CpAmount <= 0) Check.BadStatus("ServerMessage_insufficientGold");
        ...
        public bool UserChipEquals(string userId, int amountRequired)
        {
            string sql = @"SELECT (CASE WHEN EXISTS(
                           SELECT NULL AS [EMPTY]  FROM [Cps] as [c] WHERE [c].[UserId]=@UserId and [c].[CpAmount]>=@AmountRequired
                           ) THEN 1 ELSE 0 END) AS[value]";
            return Connection.Query<bool>(sql, new { UserId = userId, AmountRequired = amountRequired }).FirstOrDefault();
        }
